...
function Ready({setStep}: { setStep: Dispatch<SetStateAction<string>> }) {
  ...
  const informations = {
    ...
    ticket_type: isBrowser && localStorage.getItem('ticket_type'),
    quantity: isBrowser && localStorage.getItem('quantity'),
        ...
        <div
          ...
          <div
            ...
            <div className={'flex text-left flex-col gap-2 py-2 border-r border-b border-[#12464E]'}>
              ...
              <span
                className={'text-[12px] text-white tracking-wide'}>{informations.ticket_type && informations.ticket_type.toUpperCase()}</span>
            </div>
